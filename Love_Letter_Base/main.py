...
def draw_game_screen(game_instance):
    ...
    # Game info
    remaining = SMALL_FONT.render(f"Cards remaining: {game_instance.remainingCount()}", True, BLACK)
    WIN.blit(remaining, (20, 70))
    
    current = SMALL_FONT.render(f"Current player: {game_instance.currPlayer.name}", True, RED)
    ...
    for i, player in enumerate(game_instance.playerList):
        ...
        # Status
        status = "KNOCKED OUT" if player.isKO else "Protected" if player.hasHandmaid else "Active"
        ...
        if state == STATE_MENU:
            ...
            for event in pygame.event.get():
                ...
                if start_button and start_button.handle_event(event):
                    ...
                    player_names = [f"Player {i+1}" for i in range(selected_player_count)]
                    try:
